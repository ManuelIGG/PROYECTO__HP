...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> saveMessageToDatabase(int chatId, UserMessageModel message) async {
  ...
  Future<String> processAccountingQuery(String query) async {
    ...
    try {
      ...
      if (query.contains('total de ingresos') || query.contains('cuánto ingresó')) {
        final totalIngresos = await dbHelper.getTotalIngresos();
        response = 'El total de ingresos es de \$${totalIngresos.toStringAsFixed(2)}.';
      } 
      else if (query.contains('total de gastos') || query.contains('cuánto se gastó')) {
        final totalGastos = await dbHelper.getTotalGastos();
        response = 'El total de gastos es de \$${totalGastos.toStringAsFixed(2)}.';
      }
      else if (query.contains('balance') || query.contains('saldo') || query.contains('ganancias')) {
        final totalIngresos = await dbHelper.getTotalIngresos();
        final totalGastos = await dbHelper.getTotalGastos();
        final balance = totalIngresos - totalGastos;
        response = 'El balance actual es de \$${balance.toStringAsFixed(2)}. ' +
                   'Total de ingresos: \$${totalIngresos.toStringAsFixed(2)}. ' +
                   'Total de gastos: \$${totalGastos.toStringAsFixed(2)}.';
      }
      else if (query.contains('gastos por categoría') || query.contains('desglose de gastos')) {
        ...
        if (transacciones.isNotEmpty) {
          ...
          for (int i = 0; i < (transacciones.length > 5 ? 5 : transacciones.length); i++) {
            final t = transacciones[i];
            response += '- ${t['fecha']}: ${t['descripcion']} - \$${t['monto'].toStringAsFixed(2)} (${t['tipo']})\n';
          }
        } else {
        ...
        if (searchTerm.isNotEmpty) {
          ...
          if (resultados.isNotEmpty) {
            ...
            for (var resultado in resultados) {
              response += '- ${resultado['fecha']}: ${resultado['descripcion']} - \$${resultado['monto'].toStringAsFixed(2)} (${resultado['tipo']})\n';
            }
          } else {
        ...
        response = 'Puedo ayudarte con consultas de contabilidad como:\n' +
                  '- Total de ingresos\n' +
                  '- Total de gastos\n' +
                  '- Balance actual\n' +
